...
tam_b = size(lena_b);
pe = .45;
C = 1 + pe*log(pe)/log(2) + (1-pe)*log(1-pe)/log(2);
n = ceil(1/C);
tax = 1/n;
...
ber = sum(abs(lena_b-resp),1)/tam_b(1)*100;  
figure
im = uint8(bi2de(reshape(resp,[8 (tam_n(1))/8])'));
im = reshape(im,tam);
imshow(im)
xlabel(['n = ' num2str(n) ', ' '  BER = ' num2str(ber(end)) '$\%$' ' e $p_e = 45\%$'],'Interpreter','Latex','FontSize',14)
saveas(gcf,['n_ ' num2str(n) '_pe_45_lena.pdf'])
